...
def generate_and_get_strikes(original_strike):
    ...
    CE_STRIKES = [adjusted_ce + i for i in range(0, 1501, 50)]
    PE_STRIKES = [adjusted_pe - i for i in range(0, 1501, 50)]

    CE_SYMBOL_LIST = [
        f'NIFTY{const.EXPIRY_DATE}{const.EXPIRY_MONTH}24C{ce_strike}'
        for ce_strike in CE_STRIKES]

    PE_SYMBOL_LIST = [
        f'NIFTY{const.EXPIRY_DATE}{const.EXPIRY_MONTH}24P{pe_strike}'
...
def get_symbol(strike, option_type):
    ...
    return f'NIFTY{const.EXPIRY_DATE}{const.EXPIRY_MONTH}24{option_type}{strike}'

def rounded_to_tick(value):
    return round(round(value / 0.5) * 0.05, 2)
...
def universal_exit():
    while True:
        try:
            position = pd.DataFrame(api.get_positions())
    ...
    for position in position.itertuples():
        try:
            net_qty = int(position.netqty)
            if net_qty < 0: 
                place_order(buy_sell=const.BUY,
                            price_type=const.PRICE_TYPE_MARKET,
                            exchange=position.exch,
                            symbol=position.tysm,
                ...
                place_order(buy_sell=const.SELL,
                            price_type=const.PRICE_TYPE_MARKET,
                            exchange=position.exch,
                            symbol=position.tysm,
                            quantity=net_qty,
...
def place_straddle_and_sl():
    ...
    try:
        ...
        ce_strike = get_symbol(atm_strike, 'C')
        pe_strike = get_symbol(atm_strike, 'P')

        first_ce_strike = ce_strike
        first_pe_strike = pe_strike

        print(atm_strike, 'atm strike')
        ...
        if(ce_order_status[0]['status'] != 'COMPLETE' or pe_order_status[0]['status'] != 'COMPLETE'):
            print("Error: One or both orders are not completed. Triggering universal exit.")
            universal_exit_triggered = True
            universal_exit()
            return
        ...
        print(ce_sl_order_no,'ce order no')
        print(pe_sl_order_no, 'pe order no')
...
def place_order_for_ce(pe_token):
    ...
    try:
        ...
        print(ce_result, 'ce Result')

        if not ce_result or 'tysm' not in ce_result:
        ...
        ce_order_status = api.single_order_history(orderno=place_second_ce['norenordno'])
...
def main():
    global CE_SYMBOL_LIST, PE_SYMBOL_LIST, CE_TOKEN_LIST, PE_TOKEN_LIST, CE_WEBSOCKET, PE_WEBSOCKET, first_order_placed
    logging.basicConfig(
    ...
    token_lookup(pd.read_csv(const.OPEN_NFO_FILE),
                 CE_SYMBOL_LIST, PE_SYMBOL_LIST)

    print(CE_TOKEN_LIST, 'ce token')
    print(PE_TOKEN_LIST, 'pe token')
    ...
    def event_handler_order_update(tick_data):
        ...
        if 'remarks' in tick_data and tick_data['remarks'] == 'ce_first_sl' and 'status' in tick_data and tick_data['status'] == 'REJECTED':
            ...
            print(f"SL order for CE was rejected. {tick_data['rejreason']}")
        if 'remarks' in tick_data and tick_data['remarks'] == 'pe_first_sl' and 'status' in tick_data and tick_data['status'] == 'REJECTED':
            sl_not_placed_correctly = True
            print(f"SL order for PE was rejected. {tick_data['rejreason']}")
        ...
        if 'remarks' in tick_data and tick_data['remarks'] == 'ce_first_sl' and 'status' in tick_data and tick_data['status'] == 'COMPLETE':
            ...
            print("CE SL order completed successfully.")
        ...
        if 'remarks' in tick_data and tick_data['remarks'] == 'pe_first_sl' and 'status' in tick_data and tick_data['status'] == 'COMPLETE':
            ...
            print("PE SL order completed successfully.")
    ...
    def event_handler_feed_update(tick_data):
        ...
        try:
            ...
            print(
                f"Error updating dictionary for token {tick_data['tk']}: {e}")
    ...
    token_subscribe_ce = [f'NFO|{strike}' for strike in CE_TOKEN_LIST]
    tokens_subscribe_pe = [f'NFO|{strike}' for strike in PE_TOKEN_LIST]

    # subscribe to a single token
    total_tokens_subscribe = token_subscribe_ce + tokens_subscribe_pe
    api.subscribe(total_tokens_subscribe)
    ...
    while True:
        ...
        if universal_exit_triggered == True:
            print("Universal exit triggered. Exiting the program.")
            break
        if sl_not_placed_correctly == True:
            print("SL not placed correctly. Exiting the program.")
            break
        ...
        if pe_first_sl_hit == True and ce_first_sl_hit == False and second_iteration_executed == False:
            second_iteration_executed = True
...
if __name__ == '__main__':
    file_url = const.FINVASIA_FNO_URL
...
# NIFTY 30 JAN 25 P 28300
# feed update {'t': 'tk', 'e': 'NFO', 'tk': '52974', 'ts': 'NIFTY05DEC24P23650', 'pp': '2', 'ls': '25', 'ti': '0.05', 'lp': '18.55', 'pc': '-25.80', 'c': '25.00', 'ft': '1733133600', 'o': '27.75', 'h': '41.65', 'l': '16.65', 'ap': '25.65', 'v': '34160700', 'oi': '1816750', 'poi': '1160650', 'bp1': '18.55', 'sp1': '18.75', 'bq1': '325', 'sq1': '100'}
